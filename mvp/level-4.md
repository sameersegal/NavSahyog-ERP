...
| L4.0 | Offline platform — versioned generic outbox, IDB migration framework, build-id discipline, sync-state taxonomy, additive-only contract harness. **No new offline workflows ship in this slice.** Replay tool + cache integrity check land in L4.0e (cache integrity needs cache stores from L4.1 to be useful). | in flight (L4.0a + L4.0b + L4.0c + L4.0d in review) |
