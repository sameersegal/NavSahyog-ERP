...
| L4.1b | **Offline child creation** — `POST /api/children` joins the offline-eligible set per D35 with the visibility-after-sync rule. Same outbox plumbing; no new platform pieces. | not started |
| L4.1c | **Attendance vertical slice** — `cache_events` extension to the manifest, AttendanceForm wired through outbox, `POST /api/attendance` idempotency-keyed. | not started |
