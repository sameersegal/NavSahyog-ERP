...
// Replace-snapshot — wipe both stores in one transaction, then
// repopulate from the manifest payload. A failure mid-write leaves
// the cache empty rather than half-populated; the next pull tries
// again. Empty cache is a valid state (offline reads degrade to
// "sync to see data", same as a brand-new install).
export async function replaceSnapshot(args: {
  villages: ManifestVillage[];
  students: ManifestStudent[];
  generatedAt: number;
}): Promise<void> {
  await dbClear(['cache_villages', 'cache_students']);
  // Single transaction across both stores keeps the seed atomic
  // from the perspective of any concurrent reader (a render that
  // races a sync pull never sees a half-populated cache).
  await tx(['cache_villages', 'cache_students'], 'readwrite', async (t) => {
...
export async function wipeCache(): Promise<void> {
  await dbClear(['cache_villages', 'cache_students', 'meta']);
}
