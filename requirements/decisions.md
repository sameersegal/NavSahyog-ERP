...
1. **D1 migration** — add `clerk_user_id TEXT UNIQUE` + `clerk_synced_at INTEGER` to `user` (0011). Add `email TEXT UNIQUE-when-non-null` (0012) for the seed-bridge to look up local rows by Clerk-side email at one-shot provisioning time. Add a `pending` literal to the `role` and `scope_level` CHECK constraints (0013) so the webhook handler can INSERT a fresh local row before an admin assigns a real role/scope. Migrations are purely additive on existing rows.
2. **Worker `/auth/exchange`** — verifies Clerk JWT via `@clerk/backend` (cached JWKS), looks up `user WHERE clerk_user_id = ?`, mints the session cookie if found, returns `403 user_not_provisioned` otherwise. **Strict match on `clerk_user_id`** — no email-based self-heal on the request path, so a Clerk account on a privileged user's email cannot silently take over the local row. Reuses the existing cookie shape and `requireAuth` middleware unchanged.
3. **Worker `/webhooks/clerk`** — Svix-verified, the **single provisioning path** for new local rows. `user.created` INSERTs a row with `clerk_user_id`, `email`, `full_name`, and the `pending` role/scope sentinel (capabilities resolve to `[]`, scope to an empty village set, so the row is sign-in-able but powerless until promoted). `user.updated` refreshes `email` + `full_name` + `clerk_synced_at` on the linked row; role/scope/scope_id are never touched here — admin owns them via `PATCH /api/users/:id`. `user.deleted` invalidates active sessions and nulls `clerk_user_id` (the local row stays so audit-trail FKs keep resolving).
...
5. **Seed bridge script** — `apps/api/scripts/seed-clerk.mjs` creates Clerk users for the dummy seed accounts (`vc-anandpur`, `af-tehri`, `super-admin`, etc.) via Clerk's Backend API, captures the returned IDs, and back-fills `clerk_user_id` on the local rows. One-shot, idempotent, dev-only — uses the `email` column (0012) to pair Clerk accounts with pre-existing local rows that the webhook path never sees.
