...
| [`generated/endpoints.md`](./generated/endpoints.md) | Per-endpoint matrix grouped by bounded context: HTTP method, path, capability, roles, idempotency, offline mode. Generated by `pnpm matrix` from route `meta` blocks and `CAPABILITIES_BY_ROLE`. Do not edit by hand — `pnpm matrix:check` runs in CI. |
| [`contexts/`](./contexts/) | Bounded-context narrative — invariants, lifecycle gotchas, and cross-context coupling per slice (identity, masters, beneficiaries, programs, dashboard). Complements the matrix with what code can't express. |
